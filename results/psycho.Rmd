...
getwd()
setwd("..")
# replace 2 by length(list)
```{r}
## TODO : have the same colums number in each dataset
# change the number to load the appropriate dataset
for (i in 1){
      file <- paste(list[i], "/", list[i], "_R_syntax_file.R", sep="")
      dir <- (paste("/Users/Ofix/src/efptPWG/data/", list[i], sep=""))
      setwd(dir)
      source(paste(list[i], "_R_syntax_file.R", sep=""))
      setwd("..")
}  

# Load each data frame data in a specific data, rbind impossible since the columns size is different 
dataAlbania <- data
dataBelgium
dataBosnia-Herzegovina <- data
dataBulgaria <- data
dataCzech_Republic <- data
dataEstonia <- data
dataFinland <- data
dataFrance <- data
dataGermany <- data
...
pie(table(data$q_0005_S05A1), xlab="In a Psychotherapy training within my training programme")
```
...
temp <- c("data$q_0014_SQ002","data$q_0014_SQ003","data$q_0014_SQ004")
 ???        ??????
par(mfrow=c(1,3))
      ???                        ???       ???             ???
...
```{r}
hist(data$q_0014_SQ004, col="green",
     xlab= "Hours of Psychotherapy supervision per month",
