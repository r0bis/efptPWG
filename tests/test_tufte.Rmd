...
```{r headerGraphic, eval=TRUE, echo=FALSE,  fig.fullwidth=TRUE}
...
The Tufte-\LaTeX\ [^tufte_latex] document classes define a style similar to the style Edward Tufte uses in his books and handouts. Tufte's style is known for its extensive use of sidenotes, tight integration of graphics with text, and well-set typography. `r 2+ 2 `
...
df.test <- as.data.frame(head(mtcars[,1:4],14))
xt.test <- xtable(df.test, caption = "First  rows of mtcars")
print(xt.test, table.placement="!h")
```
## Using pander

```{r lipsum_load, echo=FALSE}

library(stringi)

```

Another option is of course the **pander** package to format \LaTeX\ tables. `r stri_rand_lipsum(1)`

```{r panderdemo, echo=FALSE}
library(pander)
pander(head(mtcars[,1:6]), caption = "First rows of mtcars")
...
One of the most prominent and distinctive features of thalign(xt.Modality) <- c('l', 'p{3.5in}')is style is the extensive use of sidenotes. There is a wide margin to provide ample room for sidenotes and small figures. Any use of a footnote will automatically be converted to a sidenote. ^[This is a sidenote that was entered using a footnote.] 
