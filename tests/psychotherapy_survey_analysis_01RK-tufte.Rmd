...
You can find ToDo list [here](https://github.com/r0bis/efptPWG/blob/master/about-data/todolist.md).[^study_todo]
...
Further interesting characteristic of our sample was to identify whether the trainees were receiving personal therapy or not. As expected most of the respondents (`r round(length(which(data_Europe_clean$q_0004 == "No")) / nrow(data_Europe_clean) * 100,2)`%) had no experience of being in personal therapy. Whether the 40 % who had had some experience in therapy were an unusually large proportion for the population of psychiatric trainees is not known. 
...
df.Q5other <- as.data.frame(data_Europe_clean$q_0005_other[!is.na(data_Europe_clean$q_0005_other)])
...
```
```{r other_responses, results='asis', echo=FALSE, eval=FALSE}
df.Q5other <- as.data.frame(data_Europe_clean$q_0005_other[!is.na(data_Europe_clean$q_0005_other)])
colnames(df.Q5other) <- c("Other answers")
xt.Q5other <- xtable(df.Q5other, caption = "Other responses to the question whether trainees have trained in psychotherapy")
align(xt.Q5other) <- c('l', 'p{4in}')
...
This is an overview of the `r length(data_Europe_clean$q_0005_other[!is.na(data_Europe_clean$q_0005_other)])` answers provided in the other option.
