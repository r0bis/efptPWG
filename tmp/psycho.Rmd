...
par(mar=c(6,3,2,3))
barplot((sort(table(data_Europe$q_0000), decreasing = TRUE))[0:23], cex.names=0.5, las=2, main="Number of answers by country", col=rainbow(23))
```

## Gender and age
```{r}
table(data_Europe$q_0001)
```
...
summary(data_Europe$q_0002)
hist(data_Europe$q_0002, col="orange",
     xlab="Age", ylab="Number", main="Age distribution", border=NA)

pie(table(data_Europe$q_0001), main ="Gender of participants")

```

## Year in Training (now the shorter labels fit in the plot)
```{r}
table(data_Europe$q_0003)
barplot(table(data_Europe$q_0003), main="Year of training", xlab="Year", las=1, col="cornflowerblue", border=NA, ylab="Number")
```
...
pie(table(data_Europe$q_0004), xlab="Receiving personal psychotherapy (present or past)")
```
...
#pie(tab)
#pie(table(data_Europe$q_0005_S05A1))
...
pie(table(data_Europe$q_0005_S05A2), 
...
pie(table(data_Europe$q_0005_S05A3), 
...
pie(table(data_Europe$q_0005_S05A4), xlab="Completed a full training in a Psychotherapy (past)")
```

## I am training in some modality
```{r}
table(is.na(data_Europe$q_0006))
pie(table(is.na(data_Europe$q_0006)),xlab="I am training in some modality of psychotherapy")
```

## Is psychotherapy important for your identity as a psychiatrist ?
```{r}
table(data_Europe$q_0007)
pie(table(data_Europe$q_0007), xlab="Psychotherapy important for the professional identity")
```

## Do you want to practise Psychotherapy ?
```{r}
table(data_Europe$q_0008)
pie(table(data_Europe$q_0008), xlab="Plan to practise Psychotherapy after training")
```
...
```{r}
table(data_Europe$q_0011_SQ001)
barplot(table(data_Europe$q_0011_SQ001), col="green", xlab="Theoretical lectures or seminars in Psychotherapy")
abline(v=1.3, lwd=3)
...
```{r}
table(data_Europe$q_0011_SQ001)
barplot(table(data_Europe$q_0011_SQ001), col="blue", xlab="Conducting Psychotherapy without supervision")
```

```{r}
table(data_Europe$q_0011_SQ003)
barplot(table(data_Europe$q_0011_SQ003), col="yellow", xlab="Conducting Psychotherapy under supervision")
```

```{r}
table(data_Europe$q_0011_SQ004)
barplot(table(data_Europe$q_0011_SQ004), col="orange", xlab="Personal Psychotherapy")
```

```{r}
table(data_Europe$q_0012)
barplot(table(data_Europe$q_0012), col="green", xlab="Are you qualified to practice Psychotherapy after your training ?")
```
...
temp <- c("data_Europe$q_0014_SQ002","data_Europe$q_0014_SQ003","data_Europe$q_0014_SQ004")
 ???        ??????
par(mfrow=c(1,3))
      ???                        ???       ???             ???
...
```{r}
barplot(table(data_Europe$q_0016_SQ001), col="light blue",
...
```{r}
hist(data_Europe$q_0018, col="light green",
