...
```{r, eval=TRUE}
names(data_mini_Europe) <- c("country", "in.training", "own.initiative", "future.practice", "satisfied", "satisfied.funding")
#str(data_mini_Europe)
...
par(mar=c(11,3,3,3))
barplot(in.training.sort$in.training, names=in.training.sort$country, las=2, main="Proportion of trainees in psychotherapy training 
        in each country", col="cornflowerblue")

oi.sort <- mini[with(mini, order(-own.initiative)), ]
barplot(oi.sort$own.initiative, names=oi.sort$country, las=2, main="Proportion of trainees in psychotherapy training 
        in each country on their own initiative", col="cornflowerblue")

future.sort <- mini[with(mini, order(-future.practice)), ]
barplot(future.sort$future.practice, names=future.sort$country, las=2, main="Proportion of trainees who would 
        like to practice psychotherapy in the future", col="cornflowerblue")

par(mar=c(11,3,3,2))
satisfied.sort <- mini[with(mini, order(-satisfied)), ]
barplot(satisfied.sort$satisfied, names=satisfied.sort$country, las=2, main="Mean satisfaction of trainees regarding 
        their psychotherapy training", col="cornflowerblue", ylim=c(-2,2))

par(mar=c(11,3,3,2))
satisfied.funding <- mini[with(mini, order(-satisfied.funding)), ]
barplot(satisfied.funding$satisfied.funding, names=satisfied.funding$country, las=2, main="Mean satisfaction of trainees regarding the  
...
c(sum(q9df[,1]=="Yes"), sum(q9df[,2]=="Yes"), sum(q9df[,3]=="Yes"), sum(q9df[,4]=="Yes"), sum(q9df[,5]=="Yes"), sum(!is.na(q9df[,6])))/574
```
...
psychotherapy"), cex.name=0.5, main="The following experience in psychotherapy 
is a mandatory part of my training in Psychiatry", cex.main=1.5)
legend("topright", inset=c(-0.27,0), legend = c("Yes", "No", "Uncertain"), fill=(terrain.colors(3)), cex=0.5, bty="n")
mandatory
```
