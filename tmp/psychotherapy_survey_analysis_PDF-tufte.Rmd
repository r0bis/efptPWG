...
```{r what_Interest, echo=FALSE, fig.cap="Therapies that trainees show interest in", results='asis'}
...
df.Q9otherInt <- head(as.data.frame(na.omit(data_Europe_clean$q_0009_other)),24)
...
```{r plot_satisfaction, fig.width=5, fig.height=2, echo=FALSE, message=FALSE, fig.fullwidth=TRUE}
# make auxiliary table to hold our likert data
sat1<-data_Europe_clean[,c("q_0019_SQ001", "q_0019_SQ002", "q_0019_SQ003")]
...
z <- likert.bar.plot(sat12, colors=c("#d7191c", "#fdae61", "#ffffbf","#abdda4","#2b83ba"),  wrap=25, centered = FALSE) + 
...
``` {r mandatoryXP, echo=FALSE, eval=FALSE}
...
df.ManXP <- data_Europe_clean[,grep("^q_0011", names(data_Europe_clean))]
colnames(df.ManXP) <- c("Lectures","TherapyNoSV","TherapyWSV","PersonalTh")
# df.ManXPNum <- df.ManXP[,1:4]
# colnames(df.ManXPNum) <- c("Lectures","TherapyNoSV","TherapyWSV","PersonalTh")
# df.ManXPA <- setNames(as.data.frame(apply(df.ManXPNum, 2, function(c)length(which(c=="Yes")))),c("countYes"))
# df.ManXPA$inTraining <- rownames(df.ManXPA)
# df.ManXPA <- df.ManXPA[,c(2,1)]

# gg.InTr <- ggplot(df.ManXP, aes(x=,y=countYes)) +
#   geom_bar(stat = "identity", fill=colours4train, width = 0.72) +
#   scale_x_discrete(name="Training in Psychotherapy status", 
#                    labels=c("YesIn" = "Yes - within my\ntraining \nprogramme","YesOut" = "Yes - outside \nof my \nprogramme","No" = "No","Completed" = "Training \ncompleted \npreviously")) +
#   ylab("Number of trainees") + 
